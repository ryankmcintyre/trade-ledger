...
- Opens the workbook, locates the named Excel table anywhere in the workbook
...
- Entry point: `write_trades(workbook_path: Path, table_name: str, trades: list[CanonicalTrade]) -> int`
  returns count of rows written

### `main.py`
- CLI entry point using `argparse`
- Arguments: `--broker [fidelity|robinhood]`, `--csv <path>`, `--workbook <path>`, `--table <name>`
